...
// 3. Find where we are and jump to the Previous note
const currentIndex = projectFiles.findIndex(f => f.file.path === currentFile.path);

if (currentIndex > 0) {
    const prevFile = projectFiles[currentIndex - 1].file;
    app.workspace.getLeaf().openFile(prevFile);
} else {
    new Notice("You are at the beginning of this project's sequence.");
}
%>